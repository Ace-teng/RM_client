...
**相关文档：** 操作手调研结论与按角色显示需求见 `docs/操作.md`；本文档 §0.6、§0.7、§5.3、§7 已将其融入需求与开发规划。**仓库内双轨实现（Python 客户端 vs React HUD 预览）及运行方式**见 §0.8。
...
| **战术分析** | 基于 DataCenter 数据（HP、经济等）计算追击/撤退建议、买活预警、经济提示；服务层计算、表现层展示。 | 5.1.5、4.4、战术分析功能设计 |
...
**问题：** 不同队员是否应使用**不同的屏幕或界面布局**？

**操作手调研结论（§0.6、§0.7，详见 `docs/操作.md`）：** 已明确五类操作角色及各自显示需求——**英雄、工程、步兵1、步兵2、云台手**。各角色对图传绘制、侧边栏、悬浮窗的布局与信息密度要求不同（如英雄侧重图传叠加与侧边栏，步兵1 需电容条/自瞄框/飞坡线等专用布局）。因此**建议采用「单屏 + 角色视图切换」或「多窗口按角色」**，在 Phase 4/5 之后实现角色视图切换。

**可能形态：**

| 方案 | 说明 |
|------|------|
| **单屏 + 视图切换** | 同一主窗口内，按角色切换「英雄视图」「工程视图」「步兵1/步兵2视图」「云台手视图」「雷达/综合视图」等，布局与控件按 §0.7 约定，数据仍来自同一 DataCenter。 |
| **多物理屏** | 一台主机接多台显示器，不同屏显示不同内容（如屏 1 图传+透视框，屏 2 雷达/态势，屏 3 控制/诊断）；或同一程序多窗口拖到不同屏。 |
| **多实例/多窗口** | 同一进程内多个 QMainWindow 或 QWidget 窗口，各自对应英雄/工程/步兵/云台手/综合，可拖到不同显示器。 |

**当前约定：** 默认描述为「单主窗口 + 中心图传 + 右侧控制面板」。**角色视图**（英雄/工程/步兵1/步兵2/云台手）按 §0.7 在 **ui 层** 实现：

- **视图模式**：主窗口内切换布局（英雄模式、工程模式、步兵1/2 模式、云台手模式、综合模式），或  
- **多窗口**：从同一 DataCenter 与事件总线读取，不同窗口展示/操作不同子集，

**不破坏**现有分层与数据流（R-ARCH-001/002/003 仍成立）。

**建议：** 在 Phase 4 最小可用 UI 基础上，Phase 5 或后续阶段增加「角色选择/视图切换」入口，并按 §0.7 为各角色实现对应布局与信息展示。
...
### 0.6 操作手调研结论与功能分级（来源：操作手调研）

以下内容来源于对操作手的调研（详见 `docs/操作.md`），作为需求优先级与功能分级的依据，**须在实现与 UI 设计中体现**。

**T0 原则（MUST）：** 原官方客户端功能均要能够实现，在此基础上进行客制化。即：**先保证与官方选手端功能对等，再扩展自定义能力。**

**功能分级：**

| 级别 | 说明 | 实现优先级 |
|------|------|------------|
| **可靠功能** | 数据来源稳定（裁判系统/车端协议），必须实现且稳定展示 | Phase 4～6 核心交付 |
| **不可靠功能** | 依赖雷达/视觉等非官方或不稳定数据源，体验增强型 | Phase 6 之后或插件实现 |

**可靠功能清单：**

- **通用（20 项）**：自瞄状态（在线/是否瞄准到目标，3 态）、超级电容状态（类体力条）、官方总能量限制、血量（含对方）、发弹量、热量限制、buff 效果（剩余时间与增益）、对方复活时间与买活金额、电机/裁判系统在线等车辆信息、己方基地扣血提示、对方基地受击反馈、等级与性能体系（己方与对方）、哨兵当前目标/选择状态/目的位置、前哨站与基地血量及前哨站重建次数、基地底装甲是否打开、发射类兵种准心（可自定义）、各兵种底盘姿态（可自定义）、受击提示、遥控器状态（丢控）、比赛时间。
- **兵种特殊（10 项）**：飞机被标记进度（己/敌）、工程机械臂姿态、步兵跳跃目标设定状态、步兵 pitch/yaw 轴角度显示、步兵自瞄目标距离、飞镖发射提示（己/敌）、步兵大小符开启时间提醒（大符含次数）、飞坡对准线、步兵状态机（翻倒自起、跳跃 on/off）。

**不可靠功能清单（可选/增强）：**

- **通用**：透视、辅助决策、高亮残血目标、对方剩余发弹量/经济、自瞄预测攻击位置、全向感知提醒。
- **兵种特殊**：英雄吊射模式黑屏后辅助 UI。

上述清单与 §5 功能需求、§7 开发阶段对应；可靠功能对应 DataCenter 与协议层字段，不可靠功能可依赖 service 层融合或插件。

### 0.7 按角色划分的显示与交互需求（来源：操作手调研）

不同兵种操作手对信息密度与布局需求不同；UI 设计须支持**按角色切换视图或布局**（与 §0.4 多屏/多角色视图一致），并遵循以下显示方式约定（详见 `docs/操作.md` 及附图）。

**显示区域约定：**

| 区域 | 说明 | 典型用途 |
|------|------|----------|
| **图传画面绘制** | 在图传画面上直接绘制图案、框、文字 | 状态条、准心、透视框、高亮、对准线等 |
| **侧边栏** | 主窗口一侧固定面板 | 等级与性能体系、哨兵目标/状态等 |
| **悬浮窗** | 可独立或叠加的小窗 | 基地底装甲、遥控器状态、全向感知等 |
| **控制面板/状态区** | 右侧或底部状态与控制区 | 与 §0.3 主界面布局一致 |

**按角色划分：**

| 角色 | 显示与布局要点 | 对应文档 |
|------|----------------|----------|
| **英雄** | 图传绘制：自瞄/电容/能量/血量/发弹/热量/buff、基地扣血/受击、前哨站与基地血量、准心/底盘姿态/受击/比赛时间等；侧边栏：等级与性能体系、哨兵目标与状态；悬浮窗：基地底装甲、遥控器状态等。部分项可开关（如飞坡对准线）。 | 操作.md、5.1、5.1.1 |
| **工程** | 通用功能齐全；兵种特殊：机械臂姿态等。布局可参考英雄，按工程所需信息密度调整。 | 操作.md、5.1 |
| **步兵 1** | 电容条（无目标时长绿条）、自瞄状态（如 notarget）、底盘状态（unlocked/locked/low/high）、紫框自瞄识别框、手打准心（「扌」型）、飞坡线、车辆状态类英雄；跳跃目标在底盘状态下；pitch 在发弹量左侧/紫框右侧，yaw 在能量条右侧；大小符提醒在哨兵图标下；自瞄目标与高亮残血等合并展示。 | 操作.md、5.1、5.1.1 |
| **步兵 2** | 与步兵 1 类似，可按队伍实际分工微调布局。 | 操作.md |
| **云台手** | 通用功能齐全即可，信息需求最多，不需额外兵种专属提示；侧重态势与全局状态。 | 操作.md、0.5 |

**UI 设计原则（与本文档一致）：** 所有角色视图从同一 DataCenter 与事件总线取数；仅布局与显示的控件/图层不同，不破坏 R-ARCH-002/003。未在操作手调研中明确注明的功能，按官方 UI 风格或本文档 §5.1 约定设计。

### 0.8 仓库内双轨实现与运行方式（当前状况）

本仓库内存在**两套独立实现**，分别用于**正式运行的桌面客户端**与 **HUD/UI 设计规范的可视化预览**。二者**无运行时依赖关系**，仅在同一仓库下并存，便于对照设计稿与实现。

| 项目 | **Python Qt 客户端（正式产品）** | **React HUD 预览（设计/规范）** |
|------|----------------------------------|----------------------------------|
| **代码位置** | `rm_client/` 下全部 Python 代码（含 `main.py`、UI、HUD 叠层、侧边栏等） | `src/` 下 React + TypeScript（`components/HUD/common/`、`styles/hud.css`、`App.tsx` 等） |
| **技术栈** | Python 3.10+、QtPy（PyQt/PySide）、QGraphicsView 图传与 HUD 绘制 | React 18、TypeScript、Vite；纯前端，在浏览器中运行 |
| **用途** | **实际使用的操作手客户端**：连接 MQTT、图传、裁判系统；图传显示、HUD、侧边栏、控制面板、诊断等均由此实现；打比赛/联调均运行此程序 | **HUD 组件与布局的规范预览**：按 §0.6、§0.7、§5.3 将通用功能组件（如自瞄状态、电容条、血条、准心、比赛时间等）实现为 React 组件，用于确认样式、位置与交互，便于与设计稿对照 |
| **数据来源** | DataCenter、MQTT、图传、demo 注入等**真实或模拟业务数据** | **写死的占位数据**（在 `App.tsx` 等处传入），仅用于展示组件外观与布局 |
| **运行方式** | 在项目根目录执行：`python -m rm_client.main --local`（或按当届启动脚本）；启动后为**桌面窗口应用** | 在项目根目录执行：`npm install`（仅首次）后 `npm run dev`；浏览器打开提示地址（如 `http://localhost:5173`）查看 **HUD 预览页** |
| **与架构文档关系** | 本文档定义的**六层架构、DataCenter、数据流、QtPy 表现层**等均针对此套实现；交付、验收、交接的**最终可运行产品**即此客户端 | 本文档中的**操作手调研功能列表、按角色显示需求、布局与样式约定**在该套实现中有对应 React 组件与 CSS，用于**设计落地与前后端/多端对齐**，不替代 Qt 客户端 |

**总结：**

- **`python -m rm_client.main --local`** → 得到的是**本架构文档所描述的正式桌面客户端**，所有功能与 UI 由 QtPy 实现。
- **`npm run dev`** → 得到的是**同一仓库下、基于 React 的 HUD 预览网站**，用于确认通用/兵种组件的视觉效果与布局，与 Python 客户端**无进程间通信、无共享状态**。
- 后续若将 HUD 迁移至 Web/Electron 或与前端协作，可基于 `src/` 的组件与样式扩展；当前赛事使用与联调仍以 Python Qt 客户端为准。
...
**示例字段（与基本功能对应）：** `game_state`（赛事状态）、`robot_states`（机器人状态）、`events`（比赛事件）、`video_frame`（图传帧）、`overlay_boxes`（透视框数据）、`calibration_result`（标定结果）、`link_status`（链路/诊断状态）、`tactical_advice`（战术建议，由 service 层 TacticalAdvisor 写入）。**本模块是全系统唯一状态源；** UI 与 service 只读写 DataCenter，禁止私有缓存。

### 4.4 service（业务层）

报警逻辑、战术计算、数据融合、推理与预测。只读写 DataCenter，不操作 UI。战术分析（TacticalAdvisor）归属本层，从 DataCenter 读取 game_state、robot_states，计算建议后写入 tactical_advice；详见 `docs/战术分析功能设计.md`。
...
| 子项 | 要求 |
|------|------|
| **数据来源** | 仅使用 DataCenter 中的 game_state、robot_states（官方协议解析结果）；禁止解析视频或非官方数据。 |
| **展示内容** | 追击/撤退/谨慎建议（基于血量差）、买活预警（屏幕中央醒目）、经济/补弹提示。 |
| **配置** | 己方阵营、血量差阈值、买活经济阈值由 config 或环境变量配置，接入真实协议后按当届规则调整。 |
| **详细设计** | 见 `docs/战术分析功能设计.md`。 |
...
### 5.3 操作手调研功能与 UI 映射（开发与设计依据）

本节将操作手调研（§0.6、§0.7，`docs/操作.md`）中的功能项映射到**实现层级、数据来源、展示位置与开发阶段**，便于开发时对照与 UI 设计落地。

**实现层级与数据流：**

| 调研功能类型 | 数据来源 | 主要归属层 | UI 归属 |
|--------------|----------|------------|---------|
| 可靠通用（血量、发弹量、能量、buff、复活时间等） | 赛事 MQTT/Protobuf → protocol → DataCenter | protocol + model | ui：图传绘制 / 侧边栏 / 控制面板 |
| 可靠兵种特殊（自瞄状态、机械臂、步兵 pitch/yaw、飞坡线等） | 车端经裁判系统分发或设备间链路 → DataCenter | protocol + model，部分需 service 计算 | ui：图传绘制 / 侧边栏 / 悬浮窗 |
| 不可靠（透视、辅助决策、高亮残血、对方经济等） | 雷达/视觉/融合 → DataCenter | service + model，可选插件 | ui：图传绘制 / HUD / 悬浮窗 |

**展示位置与角色视图对应（与 §0.7 一致）：**

| 展示位置 | 典型内容 | 实现方式（QtPy） |
|----------|----------|------------------|
| **图传画面绘制** | 自瞄/电容/能量条、准心、透视框、飞坡线、受击提示、高亮残血等 | OverlayLayer（5.1.4）；按角色视图切换绘制集 |
| **侧边栏** | 等级与性能体系、哨兵目标与状态、前哨站/基地血量等 | 控制面板或独立 QWidget 区域；按角色显示/隐藏 |
| **悬浮窗** | 基地底装甲、遥控器状态、全向感知提醒等 | QWidget 子窗口或可拖拽面板；可开关 |
| **控制面板/状态区** | 赛事指令、连接状态、诊断、通用状态汇总 | 主窗口右侧；与 §0.3 布局一致 |

**开发顺序建议（与 §7 阶段对应）：**

1. **Phase 4（最小可用 UI）**：先实现**可靠通用功能**的展示（血量、发弹量、能量、比赛时间、连接状态等），统一使用「单主窗口 + 图传 + 右侧面板」；数据从 DataCenter 读取，不区分子角色。
2. **Phase 5（态势与角色视图）**：引入**角色视图切换**（英雄/工程/步兵1/步兵2/云台手）；按 §0.7 为各角色配置图传绘制项、侧边栏与悬浮窗的显示集合；实现可靠兵种特殊功能（自瞄状态、底盘状态、准心、飞坡线等）的数据接入与绘制。
3. **Phase 6（控制与增强）**：赛事指令、标定、诊断完善；**不可靠功能**作为增强项，在 service 层或插件中实现，图传/HUD 上可选展示（如透视、高亮残血、辅助决策提示）。

按上述映射开发可保证：**先满足 T0 原则（官方功能对等 + 可靠功能全量）**，再按角色优化布局与信息密度，最后在架构内扩展不可靠/增强功能。

**无协议时的实现：** 在尚未接入真实赛事协议时，可用 **DataCenter.operator_display_state**（占位结构，见 `core/model/operator_state.py`）由 demo 或 service 层注入示例数据，供角色视图、图传 HUD、侧边栏、悬浮窗展示；协议接入后由 protocol 层解析并写入同名字段即可，**无需修改 UI 代码**。详见 `docs/开发进度与规划.md`「操作手调研 UI（无协议先行）」一节。
...
开发顺序遵循 **T0 原则**（§0.6）：先实现官方客户端对等功能与**可靠功能**，再按**角色视图**优化 UI，最后扩展**不可靠/增强功能**。各阶段目标如下。

| 阶段 | 目标 | 与操作手调研对应 |
|------|------|------------------|
| **Phase 1：基础框架** | 完成目录结构、QtPy 主窗口可启动、DataCenter 创建、事件总线 | — |
| **Phase 2：通信打通** | MQTT 可连接、图传 UDP 接收、接收数据日志打印；可选设备间链路骨架 | — |
| **Phase 3：协议解析** | Protobuf 解析成功、图传 8 字节包头与 HEVC 帧重组、数据写入 DataCenter | 为可靠功能提供数据源 |
| **Phase 4：最小可用 UI** | 图传显示、**可靠通用功能**展示（血量、发弹量、能量、比赛时间、连接状态等）；单主窗口 + 图传 + 右侧面板，暂不区分子角色 | §0.6 可靠通用 20 项中的核心项 |
| **Phase 5：态势与角色视图** | 地图、机器人位置、关键状态；**角色视图切换**（英雄/工程/步兵1/步兵2/云台手）；按 §0.7 实现各角色图传绘制、侧边栏、悬浮窗布局；**可靠兵种特殊功能**（自瞄状态、底盘状态、准心、飞坡线、电容条等）接入与绘制 | §0.7 按角色显示方式；§0.6 可靠兵种特殊 10 项 |
| **Phase 6：控制与增强** | 发送赛事指令、标定与诊断完善；**不可靠功能**（透视、辅助决策、高亮残血等）在 service/插件中实现、可选展示 | §0.6 不可靠功能；§5.1 控制面板 |
| **Phase 7：插件机制** | 动态加载插件、支持后续扩展；新功能优先以插件实现 | §4.6、§0.2 |

**UI 设计落地顺序建议：**

1. **Phase 4**：先做一套「通用」布局（图传 + 右侧状态/控制），所有可靠通用数据在同一面板展示，不区分英雄/步兵等。
2. **Phase 5**：增加「角色」下拉或切换；为英雄、步兵1 等分别实现 §0.7 约定的图传绘制项（如英雄的侧边栏等级与哨兵状态、步兵1 的电容条/自瞄框/飞坡线）；侧边栏与悬浮窗按角色显示/隐藏或切换内容。
3. **Phase 6**：在现有 HUD/图传绘制层增加可选「增强」开关（如透视、高亮残血），数据由 service 或插件写入 DataCenter，UI 只读并绘制。

按此顺序可保证：**先可用、再按角色好用、最后可扩展**，且与本文档分层及 T0 原则一致。
...
- **操作手调研文档**（`docs/操作.md`）及附图，作为功能分级与按角色 UI 设计的依据
...
| **战术分析** | §0.2、§5.1.5、§4.4、战术分析功能设计 | ✓ 追击/撤退、买活预警、经济提示 |
| **角色需求** | §0.4 多屏/多角色、§0.5 地面机器人操作手、§0.7 按角色显示 | ✓ 地面手需求；英雄/工程/步兵1/2/云台手见 §0.7、操作.md |
| **操作手调研** | §0.6 功能分级、§0.7 角色显示、§5.3 功能与 UI 映射、§7 阶段规划 | ✓ 可靠/不可靠功能、角色视图、开发顺序 |
| **产品形态** | §0.3 最终产品形态描述 | ✓ 布局、场景、非界面侧保证 |

**结论：** 基本功能与角色需求已覆盖；操作手调研（`docs/操作.md`）已融入 §0.6/0.7、§5.3、§7，实现与 UI 设计可据此按角色与优先级落地。
