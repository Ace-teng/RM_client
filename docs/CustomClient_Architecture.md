...
| **设备间通信** | socket（UDP/TCP） | 雷达、车端、自定义控制器等局域网链路。 |
...
| **雷达与设备间** | 雷达站/车端/控制器 LAN 接入；UDP/TCP、重连、心跳、丢包/延迟/带宽/最近更新时间统计；雷达位姿、透视框坐标等数据接入。 | 5.2.1、4.1、10.4 |
...
- **角色**：多设备战术信息中枢——同时接入赛事引擎（MQTT + 图传 UDP）、雷达站、地面机器人、自定义控制器等，在一个界面上完成图传观看、态势感知、标定、诊断与赛事指令下发。
- **部署**：通过 RJ45 有线接赛事引擎（192.168.12.1）；客户端 IP 由 DHCP 分配；与雷达/车端/控制器通过局域网 UDP 或 TCP 通信。
...
| **多设备协同** | 后台自动接入雷达、车端、控制器等；透视框与态势数据可来自雷达/车端；坐标与时间由系统统一管理，框与地图不漂移、不错位。 |
